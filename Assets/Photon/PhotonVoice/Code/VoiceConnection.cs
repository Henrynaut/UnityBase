        ...
        public DebugLevel GlobalSpeakersLogLevel
        {
            ...
            set
            {
                if (this.globalSpeakersLogLevel != value)
                {
                    ...
                    #if UNITY_EDITOR
                    Speaker[] speakers = FindObjectsOfType<Speaker>(); // todo: get rid of this ugliness
                    for (int i = 0; i < speakers.Length; i++)
                    {
                        if (!speakers[i].IgnoreGlobalLogLevel)
                        {
                            speakers[i].LogLevel = this.globalSpeakersLogLevel;
                        }
                    }
                    #endif
                }
