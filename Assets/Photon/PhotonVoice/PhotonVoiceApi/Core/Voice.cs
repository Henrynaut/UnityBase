...
#region nonpublic
        ...
        internal void sendConfigFrame(int targetPlayerId)
        {
            if (configFrame.Count != 0)
            {
                ...
                sendFrame0(configFrame, 0, targetPlayerId, true);
            }
        }
        ...
        internal void sendFrame0(ArraySegment<byte> compressed, FrameFlags flags, int targetPlayerId, bool reliable)
        {
            ...
            {                
                var a = new byte[compressed.Count];
                Buffer.BlockCopy(compressed.Array, compressed.Offset, a, 0, a.Length);
                configFrame = new ArraySegment<byte>(a);
